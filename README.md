...
你只需要用自然语言说清楚想看到的效果，不需要填写裁切、坐标、动画曲线或验证参数。TalkDirector 会在内部检查时间、素材来源、人物与字幕安全区；条件不足时给出明确降级方案，而不是硬做一个不可用的效果。任务中断后也会从已确认的进度继续，不让你重复提供相同信息。
...
1. **用自然语言说效果**：例如“让这段产品讲解更有节奏”，或直接粘贴一个具体 Prompt。
2. **确认视觉方案**：TalkDirector 自动识别文案、时间点、人物动作、素材来源和安全区；遇到缺失素材或构图冲突时给出可用替代方案。
