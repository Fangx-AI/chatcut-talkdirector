...
**专为已拍口播视频设计的 ChatCut 视觉导演 Skill**

读懂逐字稿、人物动作和真实画面，在真正值得强化的时刻加入重点文字、Logo、图表、网页、分屏与补充画面。

你只需要描述想看到的结果。TalkDirector 会先做一个可编辑的代表镜头，满意后再扩展到其他位置。
...
[30 秒开始](#30-秒开始) · [复制短 Prompt](#直接开始) · [查看真实效果](#先看效果) · [浏览效果参考库](VISUAL-GALLERY.md)
...
[![Prompt 003 - 品牌双模式能力对比](assets/verified-prompts/prompt-003-brand-mode-comparison.gif)](assets/verified-prompts/prompt-003-brand-mode-comparison.mp4)

**Prompt 003 · 品牌双模式能力对比**<br>
品牌图标贯穿两个信息阶段：先逐项解释两种模式，再收束为一句结果对比。
...
```text
$skill-installer install https://github.com/Fangx-AI/chatcut-talkdirector
```
...
```text
使用 $chatcut-talking-head-visual-director 分析这条口播，找出最值得加画面的 3 个时刻，先做最值得的一个给我看。
```

TalkDirector 会先给出少量高价值建议。只有在你确认后，才会生成素材、修改时间线或消耗额度。
...
TalkDirector 不要求你填写裁切坐标、动画曲线、内部 Schema 或验证清单。缺少关键信息时，它只询问一个真正影响结果的问题。
...
适合品牌对比、工具介绍和产品推荐。TalkDirector 会在内部查找真实手势、验证官方素材、保护人物与字幕，并检查入场和退场。
...
TalkDirector 不追求“特效更多”。它会保留能够增加理解、强化焦点并保护人物表达的高价值时刻，也允许画面在不需要特效时保持干净。
...
TalkDirector 可以参考 ChatCut 官方 Prompt Library 的信息结构和动效模式，再根据真实人物、字幕和安全区重新导演。官方参考不等于已验证 Recipe；只有上方 Verified Prompt Series 中的效果已经完成真实时间线验证。
...
```text
使用 $chatcut-talking-head-visual-director 分析这条口播，找出最值得加画面的 3 个时刻。
```
